...
static void run_graph()
{
    ...
    VS all_lines; // all_lines = std::vector<std::string>; each vector = one line from the file
    int line_no = 0;
    while (1) {
        // read in the next line and quit loop on EOF
        auto line = stdin_readline(); // line = std::string
        if (line.size() == 0)
            break;
        line_no++;
        ...
        auto toks = split(line); // tok = std::vector<std::string>
        if (toks.size() == 0)
            continue;
